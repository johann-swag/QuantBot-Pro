"""
================================================================================
  QuantBot Pro — Trend Following Strategy
  EMA-Crossover + RSI + ADX-Regime-Filter + ATR-Trailing-Stop

  Gleiche Interface wie MeanReversionStrategy — beide Klassen sind für
  portfolio.py austauschbar.

  Test: python3 strategies/trend_following.py
================================================================================
"""
...
# Default-Parameter (spiegeln bot.py CONFIG)
DEFAULTS = {
    "EMA_FAST":        10,
    "EMA_SLOW":        21,
    "RSI_PERIOD":      10,
...
class TrendFollowingStrategy:
    """
    Trend-Following-Strategie: EMA-Crossover + RSI-Bestätigung + ADX-Regime.

    Interface identisch zu MeanReversionStrategy — beide können von
    walk_forward.py, bot.py und portfolio.py uniform genutzt werden.
    ...
    def __init__(self, params: dict = None, verbose: bool = True):
        ...
        self.atr_mult      = p["ATR_MULTIPLIER"]
        self.rr_ratio      = p["REWARD_RISK_RATIO"]
        self.risk_pct      = p["RISK_PER_TRADE_PCT"]
        self.vol_factor    = p["VOLUME_FACTOR"]

        if verbose:
            print(
                f"TrendFollowingStrategy initialisiert | "
                f"EMA({self.ema_fast}/{self.ema_slow}) | "
                f"RSI({self.rsi_period}) Long>{self.rsi_long_min} | "
    ...
    def compute(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Fügt alle Indikatoren und Signale zum DataFrame hinzu.
        Spalten: ema_fast, ema_slow, rsi, atr, adx, vol_ma20,
        ...
        vol_ok     = df["volume"] > df["vol_ma20"] * self.vol_factor

        df["signal_long"]  = cross_up   & (df["rsi"] > self.rsi_long_min)  & regime & vol_ok
        df["signal_short"] = cross_down & (df["rsi"] < self.rsi_short_max) & regime & vol_ok
        return df
    ...
    def position_size(
        ...
        """1% Risiko pro Trade, ATR-basierter Stop, konfiguriertes R/R."""
        if atr <= 0:
        ...
        tp = entry + stop_dist * self.rr_ratio if direction == "LONG" \
             else entry - stop_dist * self.rr_ratio
        if sl <= 0:
...
if __name__ == "__main__":
    ...
    close = 80_000 + np.cumsum(rng.normal(0, 300, n))
    noise = rng.uniform(-100, 100, n)
    ...
    from strategies.trend_following import TrendFollowingStrategy as _T
    _T(verbose=False)
